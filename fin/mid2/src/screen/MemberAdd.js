    ...
    AlertIOS.prompt(
      ...
      [
        {text: 'Cancel', onPress: () => console.log('Cancel Pressed'), style: 'cancel'},
        {
          ...
          const { currentUser } = firebase.auth();
          const { name,sex,category,kind,date,type,age,number,color,sick,position,ps } = this.state;
          let dbUserid = firebase.database().ref(`/users/${currentUser.uid}`);
          dbUserid.child('miss').push({ name,sex,category,kind,date,type,age,number,color,sick,position,ps,url:"https://firebasestorage.googleapis.com/v0/b/appdesign-9e933.appspot.com/o/preload.png?alt=media&token=a54ed8f0-3a00-49ab-a739-004076a2c9c1",
              });
          this.back()
          }
        },
